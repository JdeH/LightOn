...
it is once again less practical if Python is combined with C++.
Also at the beginning of a project it is often not completely
...
The functional way requires slightly less code than the non-functional way, but that's just a bonus.
In the beginning you may prefer the non-functional way, since its explicit steps make it easier to debug.
But that will probably shift once you gain experience, since the conciseness of the functional way helps preventing bugs in the first place.
...
\lstinputlisting[caption={pong/pong1.py},label={lst:pong/pong1}]{prog/pong/pong1.py}
...
\lstinputlisting[caption={pong/pong2.py},label={lst:pong/pong2}]{prog/pong/pong2.py}
...
\lstinputlisting[caption={pong/pong3.py},label={lst:pong/pong3}]{prog/pong/pong3.py}
...
\lstinputlisting[caption={pong/pong4.py},label={lst:pong/pong4}]{prog/pong/pong4.py}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=7cm]{imag/prog/pong/pong4}
\end{center}
\caption{Class diagram of pong4.py, showing inheritance}
\end{figure}
...
\chapter{Python and performance: a wolf in sheeps clothes}
\section{The C++ connection}

Python programs can be very fast.
I've written Python applications for real time 3D medical image processing that run rings around their C++ predecessors.
It's all a matter of using the right tools for the job.
Python comes with a set of easy to use but highly optimized datastructures and standard libraries written in C++,
They are fast, memory efficient, safe and easy to use seamlessly from Python.

In addition to this, Python has become the lingua franca to use specialized high performance 3rd party libraries for number crunching, neural nets, graphics and modelling.
Let's enter the candy store (or cheese shop, as some Pythonistas would insist (Google)).

\section{Built-in datastructures and standard libraries}
...
\subsection{If Python doesn't offer it, it isn't worth having}
\subsection{Numpy/Scipy/Matplotlib}
\subsection{Tensorflow/Keras}
\subsection{OpenGL}
\subsection{Fenics/Dolphin}

\section{Ready for prime time}
